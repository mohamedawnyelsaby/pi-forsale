-- ForSale initial database setup. Paste into Supabase SQL Editor and run once.

CREATE TYPE "public"."commission_status" AS ENUM('pending', 'invoiced', 'paid', 'waived');
...
CREATE TYPE "public"."payment_purpose" AS ENUM('commission', 'promotion', 'deposit');
CREATE TYPE "public"."payment_status" AS ENUM('created', 'approved', 'completed', 'cancelled', 'failed');
CREATE TYPE "public"."plot_status" AS ENUM('available', 'reserved', 'sold');
CREATE TYPE "public"."property_kind" AS ENUM('apartment', 'villa', 'land', 'commercial', 'chalet', 'building');
CREATE TYPE "public"."user_role" AS ENUM('buyer', 'owner', 'broker', 'developer', 'admin');
CREATE TYPE "public"."verification_level" AS ENUM('none', 'documents', 'site_visit');
...
ALTER TABLE "commissions" ADD CONSTRAINT "commissions_listing_id_listings_id_fk" FOREIGN KEY ("listing_id") REFERENCES "public"."listings"("id") ON DELETE no action ON UPDATE no action;
...
ALTER TABLE "flagged_listings" ADD CONSTRAINT "flagged_listings_listing_id_listings_id_fk" FOREIGN KEY ("listing_id") REFERENCES "public"."listings"("id") ON DELETE cascade ON UPDATE no action;
...
ALTER TABLE "listing_plots" ADD CONSTRAINT "listing_plots_listing_id_listings_id_fk" FOREIGN KEY ("listing_id") REFERENCES "public"."listings"("id") ON DELETE cascade ON UPDATE no action;
...
ALTER TABLE "listings" ADD CONSTRAINT "listings_broker_id_users_id_fk" FOREIGN KEY ("broker_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;
...
CREATE INDEX "listings_geo_idx" ON "listings" USING btree ("country","city");
CREATE INDEX "listings_price_idx" ON "listings" USING btree ("currency","price_minor");
CREATE INDEX "messages_conv_idx" ON "messages" USING btree ("conversation_id","created_at");
CREATE UNIQUE INDEX "payments_pi_payment_idx" ON "payments" USING btree ("pi_payment_id");
CREATE UNIQUE INDEX "users_pi_uid_idx" ON "users" USING btree ("pi_uid");

-- Row Level Security: the app talks to Postgres directly from the server (which bypasses RLS).
-- Enabling RLS with no policies blocks all access through Supabase's public API keys.
ALTER TABLE "commissions" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "conversations" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "flagged_listings" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "listing_media" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "listing_plots" ENABLE ROW LEVEL SECURITY;
